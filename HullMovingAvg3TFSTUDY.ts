...
def HullMACD = HullMovingAvg(price, FastHullPeriod, 0) - HullMovingAvg(price, MediumHullPeriod, 0);
plot Trend = hullfast - hullslow;
plot ZeroLine = 0;

if hullfast >= hullfast[1] && hullslow >= hullslow[1] && hullsuperslow >= hullsuperslow[1] && HullMACD >= HullMACD[1]
then {
    isUP = yes;
    isDOWN = no;
    isNEUTRAL = no;
...
# def condition_up = isUP == yes && isUP[1] == no;
# alert(condition_up, "MovingAverageHull3TimePeriod: Go LONG now.", Alert.BAR, Sound.DING);

# def condition_down = isDOWN == yes && isDOWN[1] == no;
# alert(condition_down, "MovingAverageHull3TimePeriod: Go SHORT now.", Alert.BAR, Sound.DING);

# def condition_neutral = isNEUTRAL == yes && isNEUTRAL[1] == no;
# alert(condition_neutral, "MovingAverageHull3TimePeriod: Turing Neutral. Close all positions now.", Alert.BAR, Sound.DING);